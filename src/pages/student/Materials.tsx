...
export default function StudentMaterials() {
  ...
  const typeOptions = ['Hamısı', ...Array.from(new Set(materials.map((m) => m.type)))]
          ...
          <table className="w-full table-fixed border-collapse text-left">
            ...
            <thead>
              <tr className="text-xs font-semibold text-text-base/50 uppercase tracking-wide">
                <th className="pb-2 px-4 pt-4">Materialın Adı</th>
                <th className="pb-2 px-4 pt-4">Aid Olduğu Dərs</th>
                <th className="pb-2 px-4 pt-4">Tipi</th>
                <th className="pb-2 px-4 pt-4">Tarix</th>
              </tr>
            ...
            <tbody>
              {paginated.map((row, index) => (
                <tr key={row.id || index}>
                  <td className="py-3.5 text-sm text-text-base font-medium whitespace-normal break-words pr-2 px-4">{row.title}</td>
                  <td className="py-3.5 text-sm text-text-base/50 pr-2 px-4">Mövzu: {row.lesson_topic}</td>
                  <td className="py-3.5 text-sm pr-2 px-4"><MaterialTypeBadge type={row.type as any} /></td>
                  <td className="py-3.5 text-sm text-text-base/50 pr-2 px-4">{new Date(row.lesson_date).toLocaleDateString('az-AZ')}</td>
                </tr>
              ))}
              {paginated.length === 0 && (
                <tr><td colSpan={4} className="py-6 text-center text-sm text-text-base/50 px-4">Material tapılmadı.</td></tr>
              )}
            </tbody>
          </table>
        </div>
